import {structureName,type Language} from './language';
...
interface Props {language:Language;atlas:Atlas;state:SceneState;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void}
export default function AnatomyScene({language,atlas,state,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect);
 const locale=useRef(language);locale.current=language;
 useEffect(()=>{host.current?.querySelector('canvas')?.setAttribute('aria-label',language==='ar'?'تشريح تفاعلي. اسحب للدوران، وقرّب بإصبعين أو عجلة الفأرة، واضغط على بنية لفحصها.':'Interactive human anatomy. Drag to orbit, pinch or scroll to zoom, and tap a structure to inspect it.');const hover=host.current?.querySelector<HTMLElement>('.part-hover');if(hover)hover.hidden=true;},[language]);
 ...
 useEffect(()=>{
  ...
  renderer.domElement.setAttribute('aria-label',locale.current==='ar'?'تشريح تفاعلي. اسحب للدوران، وقرّب بإصبعين أو عجلة الفأرة، واضغط على بنية لفحصها.':'Interactive human anatomy. Drag to orbit, pinch or scroll to zoom, and tap a structure to inspect it.');
